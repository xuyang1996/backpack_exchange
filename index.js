...
const init = async (client) => {
  try {
    ...
    await delay(3000)
    // 判断账号USDC余额是否大于5
    if (userbalance.USDC.available > 100) {
      await buyfun(client)
    } else {
...
const sellfun = async (client) => {
  ...
  let quantitys = (userbalance2.SOL.available - 0.1).toFixed(2).toString()
...
const buyfun = async (client) => {
  console.log(getNowFormatDate(), '开始买币')
  //取消所有未完成订单
  let GetOpenOrders = await client.GetOpenOrders({ symbol: 'SOL_USDC' })
  if (GetOpenOrders.length > 0) {
    console.log(getNowFormatDate(), '存在挂单 等待2分钟')
  ...
  console.log(
    getNowFormatDate(),
    `正在买入中... 花${(userbalance.USDC.available - 2).toFixed(2).toString()}个USDC买SOL`
  )
